...
const REQUIRED_PERMISSIONS = [
	...
	},
	{
		name: "Metadata",
		access: "Read",
		reason: "Required for basic repository information (usually included automatically).",
	},
] as const;
...
export default function GitHubApiPage() {
	...
	const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
		...
		if (!cleanedToken) {
			setStatus({ kind: "error", text: "Please paste your GitHub token before submitting." });
			return;
		}
	...
	return (
		<div className="flex items-center justify-center min-h-[calc(100vh-10rem)]">
				<form onSubmit={handleSubmit} className="w-full max-w-4xl flex flex-row gap-4">
				<div className="w-3/5 px-6 md:px-8">
					...
					<ol className="mt-4 space-y-2.5 text-sm md:text-base" style={{ color: "#e5e7eb" }}>
						<li>
							<strong style={{ color: "#ffffff" }}>1. Open token settings:</strong> Go to{" "}
							<a
						...
						<li>
							<strong style={{ color: "#ffffff" }}>2. Select resource owner and repositories:</strong> Choose your
							account and grant access to the repositories you want Kaizen to manage (or all repositories).
						</li>
						<li>
							<strong style={{ color: "#ffffff" }}>3. Set repository permissions:</strong>
							<ul className="mt-1.5 list-disc pl-5 space-y-1">
								{REQUIRED_PERMISSIONS.map((permission) => (
									<li key={permission.name}>
										...
										<span style={{ color: "#d1d5db" }}> - {permission.reason}</span>
									</li>
								))}
							</ul>
						</li>
						<li>
							<strong style={{ color: "#ffffff" }}>4. Generate token:</strong> Click <em>Generate token</em> and copy
							it immediately.
						</li>
						<li>
							<strong style={{ color: "#ffffff" }}>5. Submit token here:</strong> Paste the token into the field below
							and click submit.
						</li>
					</ol>
				</div>
				...
				>
					<div className="flex flex-col gap-3">
						<label htmlFor="github-token" className="text-base md:text-lg font-medium" style={{ color: "#ffffff" }}>
						...
						{status.kind !== "idle" && (
							<p
								...
								style={{
									color: status.kind === "success" ? "var(--success-fg)" : "var(--danger-fg)",
									backgroundColor:
										status.kind === "success" ? "var(--success-subtle)" : "var(--danger-subtle)",
									border:
										status.kind === "success"
					...
					<button type="submit" className="btn-primary w-full py-2 text-sm md:text-base font-medium mt-10" disabled={isSubmitting}>
