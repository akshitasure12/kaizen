...
/** Repo must exist and be imported by the authenticated user. */
...
export async function issueRoutes(app: FastifyInstance) {
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      // Verify repo exists
      const repo = await queryOne("SELECT id FROM repositories WHERE id = $1", [
        repoId,
      ]);
      if (!repo) {
      ...
      const [inserted] = await query<Issue>(
      ...
      let issue = inserted;
      if (!parentId) {
        const gh = await createGitHubIssueForImportedRepo({
          repoId,
        ...
        if (gh.status === "error") {
          ...
          return reply.status(gh.httpStatus).send({ error: gh.message });
        }
        if (gh.status === "created") {
          const [updated] = await query<Issue>(
            `UPDATE issues SET github_issue_number = $1 WHERE id = $2 RETURNING *`,
            [gh.number, issue.id],
          );
          issue = updated;
        }
  ...
  app.get(
    "/:repoId/issues/:issueId/children",
    { preHandler: [...repoUserAuth] },
    async (req, reply) => {
  ...
  app.post(
    "/:repoId/issues/:issueId/decompose",
    { preHandler: [...repoUserAuth] },
    async (req, reply) => {
      const { repoId, issueId } = req.params as any;
      ...
      if (Number(existingChildren?.cnt || "0") > 0) {
        return reply
          .status(409)
          .send({ error: "Issue already has child issues" });
      }
      ...
      if (parentBounty && ["funded", "judging"].includes(parentBounty.status)) {
        return reply
          ...
          .send({
            error:
              "Parent issue cannot keep direct active bounty when decomposed",
          });
      }
      ...
      if (childSpecs.length < 2) {
        return reply
          .status(400)
          .send({
            error: "Provide at least two child issues for decomposition",
          });
      }
      ...
      if (totalBounty > 0 && hasPerChildBounty) {
        return reply.status(400).send({
          error:
            "Use either total_bounty_amount or per-child bounty_amount values, not both",
        });
      }
      ...
      if (willCreateBounties) {
        if (!body.poster_agent_ens) {
          return reply
            ...
            .send({
              error:
                "poster_agent_ens is required when bounty allocation is requested",
            });
        }
        const poster = await sdk.getAgent(body.poster_agent_ens);
        if (!poster || poster.user_id !== req.user!.userId) {
      ...
      if (willCreateBounties && requestedTotalBounty <= 0) {
        return reply
          .status(400)
          .send({ error: "Bounty allocation must be greater than zero" });
      }

      const childAgentByEns = new Map<
        string,
        { id: string; ens_name: string }
      >();
      const requestedChildAgents = Array.from(
        new Set(
          childSpecs
            .map((c) =>
              typeof c.agent_ens === "string" ? c.agent_ens.trim() : "",
            )
      ...
      for (const childEns of requestedChildAgents) {
        ...
        if (!childAgent || childAgent.user_id !== req.user!.userId) {
          return reply
            .status(404)
            .send({ error: `Child agent not found: ${childEns}` });
        }
      ...
      const deadlineHours =
        body.deadline_hours && body.deadline_hours > 0
          ? body.deadline_hours
          : 24;
      const maxSubmissions =
        body.max_submissions && body.max_submissions > 0
      ...
      try {
        for (let i = 0; i < childSpecs.length; i += 1) {
          ...
          const childAgentEns =
            typeof spec.agent_ens === "string"
              ? spec.agent_ens.trim().toLowerCase()
              : "";
          if (childAgentEns) {
        ...
        for (const bountyId of createdBountyIds) {
          try {
            ...
            req.log.error(
              { err: refundErr, bountyId },
              "Failed to rollback child bounty during decomposition",
            );
          }
        }
      ...
      return reply.status(201).send({
        ...
        total_bounty_amount:
          totalBounty > 0
            ? totalBounty
            : bountyAllocations.reduce((a, b) => a + b, 0),
  ...
  app.get(
    ...
    async (req, reply) => {
      ...
      let countSql = `SELECT COUNT(*)::text as c FROM issues i ${whereClause}`;
    ...
    async (req, reply) => {
      ...
      if (agent_ens) {
        ...
        if (!winner) {
          return reply
            .status(400)
            .send({ error: "No agents available for assignment" });
        }
        agentId = winner.id;
  ...
  app.get(
    "/:repoId/issues/:issueId/assignment-suggestions",
    { preHandler: [...repoUserAuth] },
    async (req, reply) => {
      const { repoId, issueId } = req.params as any;
      const issue = await queryOne<Issue>(
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      if (issue.status === "closed" || issue.status === "cancelled") {
        return reply
          .status(400)
          .send({ error: "Cannot resolve a closed/cancelled issue" });
      }
      ...
      if (
        Array.isArray(body.decomposition?.children) &&
        body.decomposition.children.length === 1
      ) {
        return reply
          .status(400)
          .send({
            error: "Provide at least two child specs for decomposition",
          });
      }
      ...
      if (body.agent_ens && body.agent_ens.trim().length > 0) {
        selectedAgent = await resolveAgentByEns(
          body.agent_ens.trim(),
          agentCache,
          ownerUserId,
        );
        if (!selectedAgent) {
          return reply
            .status(404)
            .send({ error: `Agent not found: ${body.agent_ens}` });
        }
      } else if (issue.assigned_agent_id && issue.assigned_agent_ens) {
        const ownedAssigned = await queryOne<{ id: string; ens_name: string }>(
          `SELECT id, ens_name FROM agents WHERE id = $1 AND user_id = $2`,
          [issue.assigned_agent_id, ownerUserId],
        );
        if (ownedAssigned) {
          selectedAgent = {
            id: ownedAssigned.id,
            ens_name: ownedAssigned.ens_name,
          };
          agentCache.set(ownedAssigned.ens_name.toLowerCase(), selectedAgent);
        } else {
          selectedAgent = await pickTopAgentForIssue(
            issue.title,
            issue.body || "",
            ownerUserId,
          );
          if (selectedAgent) {
            agentCache.set(
              selectedAgent.ens_name.toLowerCase(),
              selectedAgent,
            );
          }
        }
      ...
      if (!link) {
        return reply.status(400).send({
          error:
            "Repository has no GitHub remote. Import with POST /repositories/import-from-github and set PAT (PATCH /auth/github-api-key).",
        });
      }

      const baseBranch =
        body.base_branch?.trim() || link.default_branch || "main";
      const maxAttempts =
      ...
      if (resolvedPlan.path === "single_agent") {
        if (!selectedAgent) {
          return reply
            .status(400)
            .send({ error: "No available agent found for assignment" });
        }
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      if (!issue.assigned_agent_id) {
        return reply
          .status(400)
          .send({ error: "No agent assigned to this issue" });
      }
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      if (!agent_ens || !content) {
        return reply
          .status(400)
          .send({ error: "agent_ens and content are required" });
      }
   ...
   */
  app.post(
    "/:repoId/issues/:issueId/bounty",
    { preHandler: [...repoUserAuth] },
    async (req, reply) => {
      const { repoId, issueId } = req.params as any;
      const { agent_ens, amount, deadline_hours, max_submissions } =
        req.body as any;
      ...
      if (!agent_ens || !amount || !deadline_hours) {
        return reply
          .status(400)
          .send({
            error: "agent_ens, amount, and deadline_hours are required",
          });
      }
      ...
      if (deadline_hours <= 0) {
        return reply
          .status(400)
          .send({ error: "deadline_hours must be positive" });
      }
      ...
      if (!issue.parent_issue_id) {
        ...
        if (Number(childCount?.count || "0") > 0) {
          return reply.status(400).send({
            error:
              "Parent issue has child issues; post bounties on child issues only",
          });
        }
      }
      if (issue.status === "closed" || issue.status === "cancelled") {
        return reply
          .status(400)
          .send({ error: "Cannot post bounty on a closed/cancelled issue" });
      }

      // Verify agent exists and is yours (wallet spend)
      const agent = await sdk.getAgent(agent_ens);
      if (!agent || agent.user_id !== req.user!.userId) {
        return reply.status(404).send({ error: "Agent not found" });
      }
  ...
  app.get(
    ...
    async (req, reply) => {
      ...
      if (!bounty) {
        return reply
          .status(404)
          .send({ error: "No bounty found for this issue" });
      }
      ...
      const submissions = await bountyService.getIssueBountySubmissions(
        bounty.id,
      );
   ...
   */
  app.post(
    "/:repoId/issues/:issueId/bounty-submit",
    { preHandler: [...repoUserAuth] },
    async (req, reply) => {
      const { repoId, issueId } = req.params as any;
      const { agent_ens, content } = req.body as any;
      ...
      if (agent.id === bounty.poster_agent_id) {
        return reply
          .status(400)
          .send({ error: "Bounty poster cannot submit to their own bounty" });
      }

      // Check max submissions
      const currentCount = await bountyService.getBountySubmissionCount(
        bounty.id,
      );
      if (currentCount >= bounty.max_submissions) {
        return reply
          .status(400)
          .send({ error: "Maximum submissions reached for this bounty" });
      }
      ...
      try {
        ...
        if (newCount >= bounty.max_submissions) {
          if (!isRealJudge()) {
            ...
            if (issue) {
              const scorecard = issue.scorecard as Scorecard;
              triggerBountyJudging(bounty.id, issueId, scorecard).catch(
                (err) => {
        ...
        if (
          ...
          return reply
            .status(409)
            .send({ error: "Agent has already submitted to this bounty" });
        }
        return reply.status(400).send({ error: err.message });
      }
    },
   ...
   */
  app.post(
    "/:repoId/issues/:issueId/bounty-judge",
    { preHandler: [...repoUserAuth] },
    async (req, reply) => {
      const { repoId, issueId } = req.params as any;
      ...
      if (bounty.status !== "funded" && bounty.status !== "judging") {
        return reply
          .status(400)
          .send({ error: `Cannot judge bounty with status: ${bounty.status}` });
      }
      ...
      if (!isRealJudge()) {
        return reply
          .status(503)
          .send({
            error: "Live Gemini judge is required for bounty settlement",
          });
      }

      const submissionCount = await bountyService.getBountySubmissionCount(
        bounty.id,
      );
      if (submissionCount === 0) {
        ...
        await bountyService.refundIssueBounty(bounty.id);
        return {
          message: "No submissions. Bounty refunded to poster.",
          status: "refunded",
        };
      }
  ...
  app.delete(
    ...
    async (req, reply) => {
      ...
      if (bounty.poster_agent_id !== agent.id) {
        return reply
          .status(403)
          .send({ error: "Only the bounty poster can cancel" });
      }
      ...
      if (bounty.status !== "funded") {
        return reply
          .status(400)
          .send({
            error: `Cannot cancel bounty with status: ${bounty.status}`,
          });
      }
      ...
      if (submissionCount > 0) {
        return reply
          .status(400)
          .send({ error: "Cannot cancel bounty with existing submissions" });
      }
...
async function triggerBountyJudging(
  ...
  if (judgeResult.winner) {
    ...
    if (bounty) {
      await bountyService.awardIssueBounty(
        bountyId,
        judgeResult.winner.agent_id,
...
function validateScorecard(input: any): Scorecard {
  ...
  return {
    difficulty: ["easy", "medium", "hard", "expert"].includes(input.difficulty)
      ...
      : defaults.difficulty,
    base_points:
      typeof input.base_points === "number"
        ? input.base_points
        : defaults.base_points,
    unit_tests: Array.isArray(input.unit_tests)
      ? input.unit_tests
      : defaults.unit_tests,
    bonus_criteria: Array.isArray(input.bonus_criteria)
      ? input.bonus_criteria
      : defaults.bonus_criteria,
    bonus_points_per_criterion:
      typeof input.bonus_points_per_criterion === "number"
        ? input.bonus_points_per_criterion
        : defaults.bonus_points_per_criterion,
    time_limit_hours:
      typeof input.time_limit_hours === "number"
...
function normalizeRequestedChildren(
  ...
  return rawChildren
    ...
    .map((child) => ({
      ...
      estimated_effort: Math.max(
        1,
        Math.round(Number(child.estimated_effort || 1)),
      ),
...
async function resolveAgentByEns(
  ...
  if (!agent || agent.user_id !== ownerUserId) return null;
...
async function pickTopAgentForIssue(
  issueTitle: string,
  issueBody: string,
  ownerUserId: string,
): Promise<ResolvedAgent | null> {
...
async function pickAgentForChildIssue(
  ...
  if (child.assigned_agent_id && child.assigned_agent_ens) {
    const owned = await queryOne<{ id: string; ens_name: string }>(
      `SELECT id, ens_name FROM agents WHERE id = $1 AND user_id = $2`,
      [child.assigned_agent_id, ownerUserId],
    );
    if (owned) {
      const existing: ResolvedAgent = {
        id: owned.id,
        ens_name: owned.ens_name,
      };
      cache.set(owned.ens_name.toLowerCase(), existing);
      return existing;
    }
  }
